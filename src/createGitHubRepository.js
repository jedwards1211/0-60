...
async function createGitHubRepository(packageDirectory: string, options: Options = {}): Promise<any> {
  await authenticate()
  ...
  try {
    if (repositoryUrl.organization !== user) {
      return await octokit.repos.createForOrg({...props, org: repositoryUrl.organization})
    } else {
